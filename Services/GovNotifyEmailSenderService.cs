...
internal sealed class GovNotifyEmailSender(ILogger<GovNotifyEmailSender> logger,
                            ...
                            IDbContextFactory<ApplicationDbContext> contextFactory) : IGovNotifyEmailSender
{
    ...
    /// </summary>
    public async Task SendNewBridgeSurveyNotification(BridgeSurvey survey, NavigationManager navigationManager)
    {
        try
        {
            ...
            var validationLink = navigationManager.ToAbsoluteUri($"surveys/bridge/{survey.Id}/validate");
    ...
    private async Task<string> SendEmail(string emailAddress, string templateId, Dictionary<string, dynamic>? personalisation, string? clientReference = null, string? emailReplyToId = null, string? oneClickUnsubscribeURL = null)
    {
        ...
        if (personalisation != null && !environment.IsProduction())
        {
            personalisation.Add("env", $"({environment.EnvironmentName})");
        }
