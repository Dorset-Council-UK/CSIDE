...
internal class PlacesSearchService(IHttpClientFactory httpClientFactory, IOptions<MappingOptions> MappingOptions) : IPlacesSearchService
{
    ...
    public async Task<List<SimpleAddress>> GetAddresses(string searchInput)
    {
        ...
        {
            return addresses.Results.Select(x => new SimpleAddress(long.Parse(x.DPA!.UPRN!), x.DPA.Address)).ToList();
        }
        return [];
    }
    ...
    public async Task<List<SimpleAddress>> GetAddressesByGeometry(string geojson)
    {
        ...
        var apiKey = MappingOptions.Value.OSMapsAPIKey;
        var url = baseAddress;
    ...
    private static AddressSearchType GetAddressSearchTypeFromSearchInputString(string searchInput)
    {
        ...
        else if (Regex.IsMatch(searchInput, "^(([gG][iI][rR] {0,}0[aA]{2})|((([a-pr-uwyzA-PR-UWYZ][a-hk-yA-HK-Y]?[0-9][0-9]?)|(([a-pr-uwyzA-PR-UWYZ][0-9][a-hjkstuwA-HJKSTUW])|([a-pr-uwyzA-PR-UWYZ][a-hk-yA-HK-Y][0-9][abehmnprv-yABEHMNPRV-Y]))) {0,}[0-9][abd-hjlnp-uw-zABD-HJLNP-UW-Z]{2}))$"))
        {
