...
{
    public class MaintenanceInterceptor(IDbContextFactory<ApplicationDbContext> contextFactory, AuthenticationStateProvider authenticationStateProvider) : SaveChangesInterceptor, IMaintenanceInterceptor
    {
            ...
            foreach (var entry in context.ChangeTracker.Entries())
            {
                if (IsCorrectEntityType(entry) && (entry.State is EntityState.Added or EntityState.Modified))
                {
                    ...
                    if (entry.State == EntityState.Added)
                    {
                        ...
                        await SetLoggedByUserAsync((Job)entry.Entity);
                    }
                }
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task SetLoggedByUserAsync(Job job)
        {
