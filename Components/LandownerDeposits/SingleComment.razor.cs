...
namespace CSIDE.Components.LandownerDeposits
{
    ...
    {
        [Parameter]
        public required LandownerDepositComment Comment { get; set; }
        [Parameter]
        ...
        private async Task DeleteComment(int CommentId)
        {
            ...
            if (ConfirmDelete)
            {
                ...
                var commentToDelete = await context.LandownerDepositComments.FindAsync([CommentId]);
        ...
        private async Task EditComment(LandownerDepositComment comment)
        {
            if (IsBusy)
            {
                ErrorMessage = null;
            ...
            if (await editCommentValidator!.ValidateAsync())
            {
                ...
                try
                {
                    if (comment is not null)
                    {
                        ...
                        using var context = contextFactory.CreateDbContext();
                        var existingComment = await context.LandownerDepositComments.FindAsync(Comment.Id) ?? throw new Exception($"Landowner Deposit Comment being edited (ID: {Comment.Id}) was not found prior to updating");
