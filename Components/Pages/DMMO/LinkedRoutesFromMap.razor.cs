                                         ...
                                         NavigationManager navigationManager)
{
    ...
    private async Task<Data.Models.RightsOfWay.Route[]> AddOrRemoveRouteAtCoordinate(double[] coordinates)
    {
        ...
        try
        {
            ...
            //TODO if route already exists, remove it and redraw the map
            if (route is not null)
            {
                if (ExistingLinkedRoutes is not null && ExistingLinkedRoutes.Any(r => r.RouteId == route.RouteCode))
                {
            ...
            return ExistingLinkedRoutes is not null ? ExistingLinkedRoutes.Select(r => r.Route).ToArray() : [];
        }
        finally
        {
            IsBusy = false;
